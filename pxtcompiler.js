...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        var ProctoAssembler = /** @class */ (function () {
            ...
            ProctoAssembler.prototype.emitBindHelper = function () {
                var maxArgs = 12;
                ...
                this.write(pxtc.U.range(maxArgs).map(function (k) { return ".word _pxt_bind_" + k + "@fn"; }).join("\n"));
                for (var numargs = 0; numargs < maxArgs; numargs++) {
                    this.write("\n                _pxt_bind_" + numargs + ":\n                    sub sp, #4\n                ");
            ...
            ProctoAssembler.prototype.ifaceCallCore = function (numargs, getset, noObjlit) {
                ...
                var callIt = this.t.emit_int(numargs, "r4") + "\n     bx r2";
                if (getset == "set") {
            ...
            ProctoAssembler.prototype.emitCallRaw = function (name) {
                var inf = pxtc.hexfile.lookupFunc(name);
        ...
        function irToJS(bin, proc) {
            ...
            function emitProcCall(topExpr) {
                ...
                if (procid.ifaceIndex != null) {
                    ...
                    var accessor = "pxsim_pxtrt.map" + (procid.isSet ? "Set" : "Get") + "ByString";
                    if (procid.noArgs)
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        (function (decompiler) {
            ...
            function getNewName(name, takenNames, recordNewName) {
                if (recordNewName === void 0) { recordNewName = true; }
            ...
            function decompileToBlocks(blocksInfo, file, options, renameMap) {
                ...
                var checkTopNode = function (topLevelNode) {
                    ...
                    if (topLevelNode.kind === SK.FunctionDeclaration && !checkStatement(topLevelNode, env, false, true)) {
                        ...
                        if (!checkKindNamespaceDeclaration(topLevelNode, env)) {
                            ...
                            var exported = getModuleExports(topLevelNode);
                            if (env.declaredKinds[kindName]) {
                                (_a = env.declaredKinds[kindName].declaredNames).push.apply(_a, (exported.map(function (_a) {
                                    var name = _a.name;
                                    return name;
                                })));
                            }
                            ...
                            env.tileset = getModuleExports(topLevelNode).map(function (_a) {
                ...
                if (enumMembers.length || env.tileset.length) {
                    ...
                    env.tileset.forEach(function (e) {
                ...
                function attrs(callInfo) {
                    ...
                    if (blockInfo) {
                        ...
                        var parsed = pxtc.parseComments(callInfo.decl);
                        if (parsed)
                            return parsed;
                    }
                    return {
                ...
                function getIdentifier(identifier) {
                    if (isDeclaredElsewhere(identifier)) {
                        var info = pxtc.pxtInfo(identifier);
                        var id = blocksInfo.apis.byQName[info.commentAttrs.blockIdentity];
                ...
                function getImageLiteralStatement(node, info) {
                    ...
                    var nr = attributes.imageLiteralRows || 5;
                    var nleds = nc * nr;
                    ...
                    switch (n.operatorToken.kind) {
                        ...
                        case SK.PlusEqualsToken:
                            if (isTextJoin(n)) {
                                ...
                                r_1.fields = [getField("VAR", renamed)];
                                return r_1;
                            }
            ...
            function checkStatement(node, env, asExpression, topLevel) {
                    ...
                    function checkArgument(arg) {
                        ...
                        if (paramInfo.isEnum) {
                            if (checkEnumArgument(e)) {
                                ...
                                var attributes_2 = pxtc.pxtInfo(e).commentAttrs;
                                if (attributes_2 && attributes_2.enumIdentity)
                                    return undefined;
                            }
                ...
                function checkNamespaceDeclaration(n) {
                    var kindCheck = checkKindNamespaceDeclaration(n, env);
                    if (!kindCheck)
                        return undefined;
                    var tilesetCheck = checkTilesetNamespace(n);
                    if (!tilesetCheck)
                        return undefined;
                    return kindCheck;
                }
            ...
            function checkTilesetNamespace(n) {
                ...
                var fail = pxtc.Util.lf("The myTiles namespace can only export tile variables with image literal initializers and no duplicate ids");
                var commentFail = pxtc.Util.lf("Tileset members must have a blockIdentity comment and no other annotations");
                var nameRegex = new RegExp(pxt.sprite.TILE_PREFIX + "(\\d+)");
                var foundIds = [];
                var commentRegex = /^\s*\/\/%\s*blockIdentity=[^\s]+\s*$/;
                var _loop_2 = function (statement) {
                    ...
                    var commentRanges = ts.getLeadingCommentRangesOfNode(statement, statement.getSourceFile());
                    if (commentRanges && commentRanges.length) {
                        var comments = commentRanges.map(function (cr) { return statement.getSourceFile().text.substr(cr.pos, cr.end - cr.pos); }).filter(function (c) { return !!c; });
                        if (comments.length !== 1 || !commentRegex.test(comments[0])) {
                            return { value: commentFail };
                        }
                    }
                    else {
                        return { value: commentFail };
                    ...
                    if (ts.isVariableStatement(statement) && statement.declarationList.declarations) {
                        var isSingleDeclaration = statement.declarationList.declarations.length === 1;
                        var isExport = statement.modifiers && statement.modifiers.length === 1 && statement.modifiers[0].kind === SK.ExportKeyword;
                        var isConst_2 = statement.declarationList.flags & ts.NodeFlags.Const;
                        if (isSingleDeclaration && isExport && isConst_2) {
                            var declaration = statement.declarationList.declarations[0];
                            if (!declaration.initializer || !ts.isTaggedTemplateExpression(declaration.initializer) || !ts.isIdentifier(declaration.name)) {
                                return { value: fail };
                            }
                            var tag = declaration.initializer;
                            if (!ts.isIdentifier(tag.tag) || tag.tag.text !== "img") {
                                return { value: fail };
                            }
                            var match = nameRegex.exec(declaration.name.text);
            ...
            function getModuleExports(n) {
                return n.body.statements.map(function (s) {
                    var decl = s.declarationList.declarations[0];
                    return {
                        name: decl.name.text,
    ...
    (function (pxtc) {
        ...
        (function (ir) {
            ...
            var Procedure = /** @class */ (function (_super) {
                ...
                Procedure.prototype.isGetter = function () {
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function isStatic(node) {
            return node && node.modifiers && node.modifiers.some(function (m) { return m.kind == pxtc.SK.StaticKeyword; });
        }
        ...
        function compileBinary(program, opts, res, entryPoint) {
            ...
            if (opts.target.isNative) {
                ...
                pxtc.hexfile.setupFor(opts.target, opts.extinfo || pxtc.emptyExtInfo());
            ...
            function checkGetter(decl) {
                var getter = ts.getDeclarationOfKind(decl.symbol, pxtc.SK.GetAccessor);
                if (getter == null) {
            ...
            function emitShim(decl, node, args) {
                ...
                if (nm.indexOf('(') >= 0) {
                    ...
                    if (parse) {
                        ...
                        nm = parse[1];
                        if (opts.target.isNative) {
                            pxtc.hexfile.validateShim(getDeclName(decl), nm, attrs, true, litargs.map(function (v) { return true; }));
                        }
            ...
            function emitCallCore(node, funcExpr, callArgs, sig, decl, recv) {
                ...
                var isStaticLike = false;
                var noArgs = node === funcExpr;
                if (decl) {
                    switch (decl.kind) {
                ...
                if (funcExpr.kind == pxtc.SK.SuperKeyword) {
                    ...
                    for (var p = proc.classInfo.baseClassInfo; p && !baseCtor; p = p.baseClassInfo)
                        baseCtor = p.ctor;
                ...
                if (hasRecv) {
                    ...
                    if (!decl) {
                        ...
                        var fieldName = funcExpr.name.text;
                        // completely dynamic dispatch
                        return mkMethodCall(args.map(function (x) { return emitExpr(x); }), {
                    ...
                    if (recv.kind == pxtc.SK.SuperKeyword)
                        return emitPlain();
                    var needsVCall = !!info.virtualParent;
                ...
                pxtc.U.assert(!noArgs);
                return mkMethodCall(args.map(function (x) { return emitExpr(x); }), { virtualIndex: -1, noArgs: noArgs });
            }
            ...
            */
            ...
            function emitFunctionDeclaration(node) {
                ...
                if (attrs.shim != null) {
                    if (attrs.shim[0] == "@")
                        return undefined;
                    if (opts.target.isNative) {
                        pxtc.hexfile.validateShim(getDeclName(node), attrs.shim, attrs, funcHasReturn(node), getParameters(node).map(function (p) { return isNumberLikeType(typeOf(p)); }));
                    }
            ...
            function emitStore(trg, src, checkAssign) {
                ...
                if (trg.kind == pxtc.SK.Identifier || isGlobal) {
                    ...
                    if (decl_1 && (decl_1.kind == pxtc.SK.GetAccessor || decl_1.kind == pxtc.SK.SetAccessor)) {
                        checkGetter(decl_1);
        ...
        */
        ...
        (function (hexfile) {
            ...
            function setupFor(opts, extInfo) {
                ctx = cachedCtxs.find(function (c) { return c.sha == extInfo.sha; });
                if (ctx)
                    return;
                ctx = emptyCtx();
                ...
                ctx.sha = extInfo.sha;
                var hexlines = extInfo.hexinfo.hex;
                ...
                if (hexlines.length <= 2) {
                    ctx.elfInfo = pxt.elf.parse(pxtc.U.fromHex(hexlines[0]));
                    ctx.codeStartAddr = ctx.elfInfo.imageMemStart;
                    ctx.codeStartAddrPadded = ctx.elfInfo.imageMemStart;
                    var jmpIdx = hexlines[0].indexOf("0108010842424242010801083ed8e98d");
                    if (jmpIdx < 0)
                        pxtc.oops("no jmp table in elf");
                    ctx.jmpStartAddr = jmpIdx / 2;
                    ctx.jmpStartIdx = -1;
                    var ptrs = hexlines[0].slice(jmpIdx + 32, jmpIdx + 32 + funs.length * 8 + 16);
                    readPointers(ptrs);
                    checkFuns();
                    return;
                }
                ...
                var hitEnd = function () {
                    if (!ctx.codeStartAddr) {
                        var bytes = parseHexBytes(hexlines[lastIdx]);
                        ...
                        var paddingBytes = ctx.codeStartAddrPadded - ctx.codeStartAddr;
                        pxtc.assert((paddingBytes & 0xf) == 0);
                        ctx.codePaddingSize = paddingBytes;
                    }
                };
                for (; i < hexlines.length; ++i) {
                    var m = /:02000004(....)/.exec(hexlines[i]);
                    if (m) {
                        upperAddr = m[1];
                    }
                ...
                ctx.funcInfo = {};
                for (var i_3 = ctx.jmpStartIdx + 1; i_3 < hexlines.length; ++i_3) {
                    var m = /^:..(....)00(.{4,})/.exec(hexlines[i_3]);
                    if (!m)
                        continue;
                    readPointers(m[2]);
                    if (funs.length == 0)
            ...
            function hexTemplateHash() {
                var sha = ctx.sha ? ctx.sha.slice(0, 16) : "";
                while (sha.length < 16)
                    sha += "0";
            ...
            function patchHex(bin, buf, shortForm, useuf2) {
                var myhex = ctx.hexlines.slice(0, ctx.codeStartIdx);
                ...
                buf[20] = bin.commSize;
                var zeros = [];
                for (var i = 0; i < ctx.codePaddingSize >> 1; ++i)
                    zeros.push(0);
                ...
                var hd = [0x4210, 0, ctx.codeStartAddrPadded & 0xffff, ctx.codeStartAddrPadded >>> 16];
                ...
                if (ctx.elfInfo) {
                    ...
                    var resbuf = pxt.elf.patch(ctx.elfInfo, prog);
                ...
                if (!shortForm) {
                    var app = ctx.hexlines.slice(ctx.codeStartIdx);
                    if (uf2)
        ...
        function vtableToAsm(info, opts, bin) {
            ...
            */
            ...
            for (var _b = 0, _c = info.itable; _b < _c.length; _b++) {
                var e = _c[_b];
                offsets[e.idx + ""] = offset;
                var desc = !e.proc ? 0 : e.proc.isGetter() ? 1 : 2;
        ...
        function assembleAndPatch(src, bin, opts, cres) {
            ...
            var checksumWords = 8;
            var pageSize = hexfile.flashCodeAlign(opts.target);
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function createSymbolInfo(typechecker, qName, stmt) {
            ...
            if (kind != 0 /* None */) {
                var decl = stmt;
                var attributes_3 = pxtc.parseComments(decl);
                if (attributes_3.weight < 0)
                    return null;
                var m = /^(.*)\.(.*)/.exec(qName);
                ...
                var r = {
                    ...
                    parameters: !hasParams ? null : pxtc.Util.toArray(decl.parameters).map(function (p, i) {
                        ...
                        var desc = attributes_3.paramHelp[n] || "";
                        var minVal = attributes_3.paramMin && attributes_3.paramMin[n];
                        var maxVal = attributes_3.paramMax && attributes_3.paramMax[n];
                        ...
                        if (p.type && p.type.kind === pxtc.SK.FunctionType) {
                            ...
                            if (attributes_3.mutate === "objectdestructuring") {
                        ...
                        if (attributes_3.block && attributes_3.paramShadowOptions) {
                            var argNames_1 = [];
                            attributes_3.block.replace(/%(\w+)/g, function (f, n) {
                                argNames_1.push(n);
                                return "";
                            });
                            if (attributes_3.paramShadowOptions[argNames_1[i]]) {
                                options['fieldEditorOptions'] = { value: attributes_3.paramShadowOptions[argNames_1[i]] };
                            }
                        }
                        if (minVal)
                        ...
                        return {
                            ...
                            initializer: p.initializer ? p.initializer.getText() :
                                pxtc.getExplicitDefault(attributes_3, n) ||
                                    (p.questionToken ? "undefined" : undefined),
                            default: attributes_3.paramDefl[n],
                            properties: props,
        ...
        (function (service_1) {
            ...
            var operations = {
                ...
                pydecompile: function (v) {
                    host.setOpts(v.options);
                    return pxtc.transpile.tsToPy(service.getProgram(), v.fileName);
                },
                assemble: function (v) {
                    return {
            ...
            function getSnippet(apis, takenNames, runtimeOps, fn, decl, python) {
                ...
                if (attrs && attrs.blockSetVariable) {
                    if (python) {
                        var varName = pxtc.snakify(attrs.blockSetVariable);
